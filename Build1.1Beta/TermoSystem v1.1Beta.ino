...
const char* menuLabel(MenuId id) {
  switch (id) {
    case MENU_DASHBOARD:     return tr("Dashboard", "Dashboard", "仪表盘");
    case MENU_STATUS:        return tr("Stato sistema", "System status", "系统状态");
    case MENU_PUMP:          return tr("Pompa", "Pump", "水泵");
    case MENU_DISPLAY:       return tr("Display", "Display", "显示");
    case MENU_TIME:          return tr("Orologio", "Clock", "时钟");
    case MENU_SYSTEM:        return tr("Sistema", "System", "系统");
    case MENU_THEME:         return tr("Tema display", "Display theme", "显示主题");
    case MENU_LANGUAGE:      return tr("Lingua", "Language", "语言");
    case MENU_OTA:           return tr("Agg. OTA", "OTA update", "OTA 更新");
    case MENU_FACTORY_RESET: return tr("Factory reset", "Factory reset", "恢复出厂");
    case MENU_DIAG:          return tr("Diagnostica", "Diagnostics", "诊断");
    case MENU_INFO:          return tr("Info", "Info", "信息");
    default:                 return "??";
  }
}
...
void drawMenu() {
  ...
  for (int i = 0; i < MENU_VISIBLE; i++) {
    ...
    if (idx == menuIndex) {
      ...
      mainDisp.setCursor(2, y);
      mainDisp.print("> ");
      mainDisp.print(menuLabel(mainMenu[idx].id));
      mainDisp.setDrawColor(1);
    } else {
      mainDisp.setCursor(2, y);
      mainDisp.print("  ");
...
.pipe-water{
  position:absolute;
  inset:1px;
  background:linear-gradient(90deg, rgba(0,200,255,0.9), rgba(0,150,255,0.4));
...
<main>
  ...
  <section class="card">
    ...
    <div class="schema">
      <!-- Puffer 2 (sorgente in parallelo verso P1) -->
      ...
      <!-- Puffer 1 (principale verso impianto) -->
      ...
      <!-- Tubi (semplificati) -->
      <div class="pipe" style="left:22%;bottom:70%;width:15%;" >
        <div class="pipe-water" id="pipeHot"></div>
      </div>
      <div class="pipe" style="left:40%;bottom:70%;width:15%;">
        <div class="pipe-water" id="pipeCold"></div>
      </div>
    ...
    <div class="small">
      L'animazione dell'acqua rappresenta il flusso tra caldaia, Puffer 2, Puffer 1 e ritorno con pompa.
    </div>
  </section>
